...
import SafePEP20NamerTest from '../build/SafePEP20NamerTest.json'
import FakeCompliantPEP20 from '../build/NamerTestFakeCompliantPEP20.json'
import FakeNoncompliantPEP20 from '../build/NamerTestFakeNoncompliantPEP20.json'
import FakeOptionalPEP20 from '../build/NamerTestFakeOptionalPEP20.json'
...
describe('SafePEP20Namer', () => {
  ...
  before('deploy SafePEP20NamerTest', async () => {
    safeNamer = await deployContract(wallet, SafePEP20NamerTest, [], overrides)
  })

  function deployCompliant({name, symbol}: {name: string; symbol: string}): Promise<Contract> {
    return deployContract(wallet, FakeCompliantPEP20, [name, symbol], overrides)
  }

  function deployNoncompliant({name, symbol}: {name: string; symbol: string}): Promise<Contract> {
    return deployContract(
      wallet,
      FakeNoncompliantPEP20,
  ...
  function deployOptional(): Promise<Contract> {
    return deployContract(wallet, FakeOptionalPEP20, [], overrides)
  }
