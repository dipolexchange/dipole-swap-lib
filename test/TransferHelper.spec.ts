...
import FakePEP20Noncompliant from '../build/TransferHelperTestFakePEP20Noncompliant.json'
import FakePEP20Compliant from '../build/TransferHelperTestFakePEP20Compliant.json'
...
describe('TransferHelper', () => {
  ...
  before(async () => {
    ...
    fakeNoncompliant = await deployContract(wallet, FakePEP20Noncompliant, [], overrides)
    fakeCompliant = await deployContract(wallet, FakePEP20Compliant, [], overrides)
  })
  ...
  describe('#safeTransferLAT', () => {
    it('succeeds call not reverted', async () => {
      await fakeFallback.setup(false)
      await transferHelper.safeTransferLAT(fakeFallback.address, 0)
    })
    it('fails if call reverts', async () => {
      await fakeFallback.setup(true)
      await expect(transferHelper.safeTransferLAT(fakeFallback.address, 0)).to.be.revertedWith(
        'TransferHelper: LAT_TRANSFER_FAILED'
      )
    })
  })
})
